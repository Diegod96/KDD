...
def preModeling(df):
    ...
    i = ['encounter_id', 'patient_nbr', 'gender', 'admission_type_id', 'discharge_disposition_id',
         ...
         'age', 'A1Cresult', 'max_glu_serum', 'level1_diag1', 'level1_diag2', 'level1_diag3', 'level2_diag1',
...
def logisticRegression(df_pd):
    feature_set = ['age', 'time_in_hospital', 'num_procedures', 'num_medications', 'number_outpatient_log1p',
                   ...
                   'admission_type_id_3', 'admission_type_id_5', 'discharge_disposition_id_2',
                   ...
                   'num_medications|numchange', 'level1_diag1_1.0', 'level1_diag1_2.0', 'level1_diag1_3.0',
                   'level1_diag1_4.0',
    ...
    print("Logistic Regression Statistical Information:")
    print('--------------------------------------\n')
    X = df_pd[feature_set]
    y = df_pd['readmitted']
    print("Readmitted [0] vs. Not-Readmitted [1]")
    ...
    print('\nOriginal data set shape {}'.format(Counter(y_train)))
    sm = SMOTE(random_state=20)
    train_input_new, train_output_new = sm.fit_sample(X_train, y_train)
    print('New data set shape {}'.format(Counter(train_output_new)))
    ...
    return accuracy_logit, precision_logit, recall_logit


def decisionTree(df_pd):
    feature_set = ['age', 'time_in_hospital', 'num_procedures', 'num_medications', 'number_outpatient_log1p',
                   'number_emergency_log1p', 'number_inpatient_log1p', 'number_diagnoses', 'metformin',
                   'repaglinide', 'nateglinide', 'chlorpropamide', 'glimepiride', 'glipizide', 'glyburide',
                   'pioglitazone', 'rosiglitazone', 'acarbose', 'tolazamide', 'insulin', 'glyburide-metformin',
    ...
    print("\n\nDecision Tree Statistical Information:")
    print('--------------------------------------')
    ...
    print('\nOriginal data set shape {}'.format(Counter(y)))
    smt = SMOTE(random_state=20)
    train_input_new, train_output_new = smt.fit_sample(X, y)
    print('\nNew data set shape {}'.format(Counter(train_output_new)))
    ...
    print("\nMost Important Features Calculated to Use For Decision Tree:")
    print("------------------------------------------------------------")
    ...
    return accuracy_dtree, precision_dtree, recall_dtree


def randomForest(df_pd):
    feature_set = ['age', 'time_in_hospital', 'num_procedures', 'num_medications', 'number_outpatient_log1p',
                   'number_emergency_log1p', 'number_inpatient_log1p', 'number_diagnoses', 'metformin',
                   'repaglinide', 'nateglinide', 'chlorpropamide', 'glimepiride', 'glipizide', 'glyburide',
                   'pioglitazone', 'rosiglitazone', 'acarbose', 'tolazamide', 'insulin', 'glyburide-metformin',
    ...
    print("\n\nRandom Forest Statistical Information:")
    print('--------------------------------------')
    X = df_pd[feature_set_no_int]
    y = df_pd['readmitted']
    ...
    print("\nAccuracy is {0:.2f}".format(accuracy_score(y_test, rm_prd)))
    ...
    return accuracy_rm, precision_rm, recall_rm


def comparisonPlot(accuracy_logit, accuracy_dtree, accuracy_rm, precision_logit, precision_dtree, precision_rm, recall_logit, recall_dtree, recall_rm):
...
def main():
    raw_diabetes = importData('diabetic_data.csv')
    clean_diabetes = dropMissing(raw_diabetes)
    feature_diabetes = featureEngineering(clean_diabetes)
    df_pd = preModeling(clean_diabetes)
    accuracy_logit, precision_logit,recall_logit = logisticRegression(df_pd)
    accuracy_dtree, precision_dtree,recall_dtree = decisionTree(df_pd)
    accuracy_rm, precision_rm,recall_rm = randomForest(df_pd)
    comparisonPlot(accuracy_logit, accuracy_dtree, accuracy_rm, precision_logit, precision_dtree, precision_rm, recall_logit, recall_dtree, recall_rm)
